// CS 2690 Program 3
// Simple Windows Sockets UDP Echo Client (IPv6)
//Blake Thompson CS2690 Section 001 10/26/2020
...
void main(int argc, char* argv[])
{
	...
	int sock;
	__try { sock = socket(AF_INET6, SOCK_DGRAM, IPPROTO_UDP); }
	...
	int sent = sendto(sock, argv[3], msgLen, 0, (struct sockaddr *) &serverInfo, sizeof(serverInfo));
	if (sent != msgLen) {
		...
		do {
			...
			fromInfo.sin6_family = AF_INET6;
			// if recv fails, DisplayFatalErr() with appropriate message as before
			byteTracker = recvfrom(sock, &rcvBuffer, RCVBUFSIZ - 1, 0, &fromInfo, &fromSize);
